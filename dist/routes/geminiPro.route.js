...
router.route("/").get(async (req, res) => {
    ...
    const rawChatMessages = req.query.chatMessages;
    const rawActiveAI = req.query.activeAI;
    const chatMessages = JSON.parse(rawChatMessages);
    const activeAI = JSON.parse(rawActiveAI);
    console.log(chatMessages);
    console.log(activeAI);
    const result = await runGeminiPro(req, res, chatMessages, activeAI, (chunkText) => {
...
const runGeminiPro = async (req, res, chatMessages, activeAI, chunkCallback) => {
    const model = genAI.getGenerativeModel({
        model: "gemini-pro",
        safetySettings,
    });
    const [systemMessage, history, userNewMessage] = getHistoryAndNewMessage(chatMessages, activeAI);
    ...
    try {
        ...
        for await (const chunk of result.stream) {
            ...
            if (chunk.promptFeedback &&
                ...
                throw new GeminiProcessingError(`Safety Block Detected (non stream) ${chunk.promptFeedback.blockReason} = ${chunk.promptFeedback.safetyRatings}`);
            }
            if (chunk.candidates && chunk.candidates[0].finishReason !== "STOP") {
                ...
                throw new GeminiProcessingError(`Google Block Detected while Streaming="${chunk.candidates[0].finishReason}"`);
            }
...
//# sourceMappingURL=geminiPro.route.js.map